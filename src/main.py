...
def pick_target(df: pd.DataFrame) -> str:
    ...
    for k in ["status", "acquired", "label", "is_success", "success"]:
        if k in cols:
            return cols[k]
    raise ValueError("target column not found. expected one of status or acquired or label or is_success or success")

def map_target(y_raw: pd.Series) -> pd.Series:
    y = y_raw.astype(str).str.lower().str.strip()
    y = y.map(lambda v: 1 if v in ["acquired", "ipo", "1", "true", "success", "successful"] else 0)
    return y.astype(int)
...
def add_light_features(X: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in list(X.columns):
        if X[c].dtype == object:
            if any(k in c.lower() for k in ["founded", "first", "last", "closed", "ipo", "year", "date"]):
    ...
    def sector_prior(row, text_cols):
        ...
        s = 1
        if any(w in text for w in ["fintech", "finance", "payment", "payments", "wallet", "lending", "insurance", "trading", "crypto", "bank"]):
            s = 3
    ...
    text_cols = [c for c in X.columns if any(k in c.lower() for k in ["category", "market", "industry", "sector", "type", "tags"])]
...
def choose_threshold(y_true: np.ndarray, p: np.ndarray, target_acc: float = 0.96) -> Tuple[float, float]:
...
def export_threshold_sweep(y_true: np.ndarray, p: np.ndarray, path: str) -> None:
...
def export_reliability_plot(y_true: np.ndarray, p: np.ndarray, path: str) -> None:
...
pipe.fit(X_tr, y_tr)
    ...
    print("confusion matrix order tn fp fn tp")
    ...
    bucket = pd.cut(risk, bins=[-1, 33, 66, 101], labels=["Low", "Medium", "High"]).astype(str)

    name_cols = [c for c in df.columns if any(k in c.lower() for k in ["name", "company", "org", "startup"])]
